...
class SGLangBackend:
    ...
    def get_pause_requests(self) -> list[HttpRequest]:
        ...
        SGLang requires before releasing memory. The second request raises the
        scheduler's paused flag so the colocate loop services awex work; by then
        there is nothing left for it to retain.
        """
        return [
